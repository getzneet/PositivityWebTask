$(document).ready(function() {
	...
	var TrialsPerCondition = 1;//30;//1;
	var Questionnaire = 1;
	var MaxTrainingSessions = 2;
	var StatesPerCondition = 1;
	var PostLearningFeedback = 1;
	var InterLeaved = 1;

	var imgPath = 'src/assets/images/cards_gif/stim/';
	var nImg = 12;
	var imgExt = 'gif';

	// Instantiate the gui component
	var gui = new Gui({imgPath: imgPath, nImg: nImg, imgExt: imgExt});
	gui.init();

	// feedback duration
	var fb_dur = 2000;
	var fbpost_dur = 500 + 1500 * PostLearningFeedback;
	var border_color = "transparent";

	/* On calcule les différentes conditions possibles */
	/* contingencies (x2), magnitude, valence, information */
	var Conditions = [[0.75,0.25,10,1,0,],[0.75,0.25,1,1,0,],]; /* on ajoute 2 conditions après qui sont les mêmes (2 fois 10c, 2 fois 1£) */

	var TrainingConditions = Conditions.slice(0);
	...
	for (j = 0; j < StatesPerCondition; j++) {
		...
		for (i = 0; i < Conditions.length; i++) {
			Conds.push(Conditions[i],Conditions[i]); /* les conditions vont etre à la suite: soit 10c - 10c, soit 1euro - 1euro */
		}
	}
	...
	fbs = ["empty","0","1","-1",'10','-0.1',"cf_0","cf_1","cf_-1","cf_10","cf_-0.1"];
	...
	for(var i = 0;i<4;i++){
		for(var r = 0; r<NbPostLearningTrials/2; r++){ /*montrer r fois chaque paire dans un ordre particulier*/
			if(i<2){
				PLOptions.push([AllOptions[i],AllOptions[i+4]], [AllOptions[i+4],AllOptions[i]]);
				PLOptionValues.push([AllOptionValues[i],AllOptionValues[i+4]], [AllOptionValues[i+4],AllOptionValues[i]]);
				PLOptionsOrder.push([AllOptionsOrder[i],AllOptionsOrder[i+4]], [AllOptionsOrder[i+4],AllOptionsOrder[i]]);
				PLOptionsCond.push([i+5],[i+5]);
				indexes.push(k);
				k++;
				indexes.push(k);
				k++;
			}else{
				PLOptions.push([AllOptions[i],AllOptions[9-i]], [AllOptions[9-i],AllOptions[i]]);
				PLOptionValues.push([AllOptionValues[i],AllOptionValues[9-i]], [AllOptionValues[9-i],AllOptionValues[i]]);
				PLOptionsOrder.push([AllOptionsOrder[i],AllOptionsOrder[9-i]], [AllOptionsOrder[9-i],AllOptionsOrder[i]]);
				PLOptionsCond.push([i+5],[i+5]);
				indexes.push(k);
				k++;
				indexes.push(k);
	...
	for(var i = 0;i<indexes.length;i++){
		...
		PostLearningOptionsCond.push(PLOptionsCond[indexes[i]]);
	}
	...
	function PlayTraining(TrialNum) {
		...
		var Cond = (TrainingSession[TrialNum][2] +1)*2-1;
	...
	function PlayOptions(SessionNum,TrialNum) {
		...
		var Cond = Sessions[SessionNum][TrialNum][2] +1;
